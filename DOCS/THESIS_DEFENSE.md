...
- Кнопка **«Проверить похожие темы (ИИ)»**: сервер отправляет формулировку и список тем кафедры в **OpenAI** (модель из `OPENAI_MODEL`, ключ только в `.env`); в ответе — **процент схожести** и краткое пояснение по каждой похожей теме.
- Поля **год выпуска** и **группа** студента; увеличенный базовый шрифт интерфейса.
- **История** — кнопка у темы и блок в модалке редактирования (кто создал/изменил, diff полей).
- Предупреждение о **точном дубликате** (нормализация + уникальный индекс в БД) дополняет семантическую проверку ИИ.
...
1. **Веб-интерфейс** с разграничением **администратор / оператор кафедры**.  
2. **Учёт тем ВКР** (модалки, год выпуска, группа).  
3. **Контроль повторяемости:** точные дубликаты + **ИИ с процентом схожести** в пределах кафедры.  
4. **История изменений** тем (`topic_audit_log`).  
5. **Резервное копирование** БД (скрипт и скачивание для админа).  
6. **Защита данных** — см. [`SECURITY.md`](./SECURITY.md): RBAC, серверные API, секреты в env, аудит.
...
| Файл | Содержание |
|------|------------|
| [`DOCS/USER_GUIDE.md`](./USER_GUIDE.md) | Пошагово: что нажимать, настройка `.env` и миграции |
| [`DOCS/SECURITY.md`](./SECURITY.md) | Как реализована защита данных |
